...
$(document).ready(function() {
	...
	var size = 234; //($(".left").css("width").slice(0, 3) - 20) / 2 ;

	// put random letters on the board
	for (var i = 0; i < size; i++) {
		$(".letters").append("<span class='filsonpro letra " + (i + 1) + "'>" + 
	...
	for (var i = 0; i < words.length; i++) {
		...
		$(".words").append("<span class='filsonpro " + (i) + "'>" +  
							words2[i].word + "</span>");	
		pos2 = pos;
	}
    ...
    $('#calificar').on('click', calificaTurn);
})
...
function displayWord(w) {
	for (var j = 0; j < w.word.length; j++){
		if (w.direction == "N") {
			...
			if (j + 1 != w.word.length) w.end -= largo;
		}
		if (w.direction == "NE") {
			$(".letters").find("." + w.end).text(w.word[j]);
			if (j + 1 != w.word.length) w.end -= (largo-1);
		}
		if (w.direction == "E") {
			$(".letters").find("." + w.end).text(w.word[j]);
...
let radio = 9;/*aqui se deside el grosor de barra que se usa para marcar las palabras*/
let otros = 18;/*en el style esta la clase '.letters span' el width y el height deben ser iguales entre ellos e igual a esta variable*/
let interlineado = (otros/2); /*esta variable debe ser igual a la mitad de la cantidad de px  de el width de la clase '.letters span'*/
let color1 = "#c3c3008a";
let color2 = "#c3c300ab";
let total = 0;
let puntajes = 0;
...
$(document).ready(function() {
	$("#c").on("mousedown mouseup mousemove mouseleave", function(e) {
		...
		for (var i = 0; i < pos2.length; i++) {
			if (pos2[i].start === 0 && pos2[i].end === 0) {
				pos2.splice(i, 1);
			}
		}
		//console.log(e);
      ...
      // Used for Firefox
			...
			sX -= (sX % interlineado);
			sY -= (sY % interlineado);
			if (!(sX % otros)) sX += interlineado;
			if (!(sY % otros)) sY += interlineado;
			...
			if (mouseMoved) {
				...
				eX -= eX % interlineado;
				eY -= eY % interlineado;
				if (!(eX % otros)) eX += interlineado;
				if (!(eY % otros)) eY += interlineado;
				...
				if (checkWord()) {
					...
					if(isEndOfGame()) {
						/*alert("Good job!");*/
						// $(".letra").addClass('final');
						// $('.img_res').html('<img src="img/fuegos-artificiales.png" style="max-width: 90%;">');
				        // $('.btns_modal').html('<button type="button" class="btn" data-bs-dismiss="modal" style="margin-right: 5px; background: var(--bg-color);width: fit-content;color: #fff;"><h3 class="m-0">cerrar</h3></button>');
				        // $('#texto-modal').html('<b style="color: var(--bg-color);">¡FELICITACIONES!</b><br>Su puntaje es <b style="color: var(--bg-color);">100%</b>');
				        // $('#modal-ahorcado').modal("show");
				        // $('#calificar').addClass("d-none");
...
function draw(f) {
	...
	if (f == "mousedown"){
		...
		*/
		// NW
		if ((limit > 1 && limit < 6) && (eX < sX && eY < sY)) {/*NW*/
			if (f == "mousemove") ctx.clearRect(0, 0, width, height);
			diff = sX - eX;
		...
		// NE
		if ((limit < -1 && limit > -6) && (eX > sX && eY < sY)) {/*NE*/
			if (f == "mousemove") ctx.clearRect(0, 0, width, height);
			diff = sX - eX;
		...
		// SW
		if ((limit < -1 && limit > -6) && (eX < sX && eY > sY)) {/*SW*/
			if (f == "mousemove") ctx.clearRect(0, 0, width, height);
			diff = sX - eX;
		...
		// SE
		if ((limit > 1 && limit < 6) && (eX > sX && eY > sY)) {/*SE*/
			if (f == "mousemove") ctx.clearRect(0, 0, width, height);
			diff = sX - eX;
...
function setPos(x, y, loc) {
	tX = Math.floor((x / (8)) / (otros/8) ) + 1;
	tY = Math.floor((y / (8)) / (otros/8) ) + 1;
	if (loc == "start") click.startPos = (tY - 1) * largo + tX;
	else click.endPos = (tY - 1) * largo + tX;
}
...
var calificaTurn = function () {
		console.log('entro');
        puntajes = ((pos2.length * 100)/words.length).toFixed(0);
        puntajes = -puntajes + 100;
        if (puntajes >= 80) {
          $('.img_res').html('<img src="img/fuegos-artificiales.png" style="max-width: 90%;">');
          $('.btns_modal').html('<button type="button" class="btn" data-bs-dismiss="modal" style="margin-right: 5px; background: var(--bg-color);width: fit-content;color: #fff;font-size: 22px;font-weight: bold;"><p class="m-0">INTÉNTELO NUEVAMENTE</p></button>');
          $('#texto-modal').html('<b style="color: var(--bg-color);">¡FELICITACIONES!</b><br>Su puntaje es <b style="color: var(--bg-color);">'+puntajes+'%</b>');
          $('#modal-ahorcado').modal("show");
        }
        if (puntajes < 80) {
          $('.img_res').html('<img src="img/mala-critica.png" style="max-width: 90%;">');
          $('.btns_modal').html('<button type="button" class="btn" data-bs-dismiss="modal" style="margin-right: 5px; background: var(--bg-color);width: fit-content;color: #fff;font-size: 22px;font-weight: bold;"><p class="m-0">INTÉNTELO NUEVAMENTE</p></button>');
          $('#texto-modal').html('Su puntaje es <b style="color: var(--bg-color);">'+puntajes+'%</b>');
          $('#modal-ahorcado').modal("show");
        }
		if (puntajes == 100) {
			$(".letra").addClass('final');
						  $('.img_res').html('<img src="img/fuegos-artificiales.png" style="max-width: 90%;">');
						  $('.btns_modal').html('<button type="button" class="btn" data-bs-dismiss="modal" style="margin-right: 5px;background: var(--bg-color);width: fit-content;color: #fff;margin-top: -5px;padding-top: 0px;padding-bottom: 4px;font-size: 22px;font-weight: bold;"><p class="m-0">CERRAR</p></button>');
						  $('#texto-modal').html('<b style="color: var(--bg-color);">¡FELICITACIONES!</b><br>Su puntaje es <b style="color: var(--bg-color);">100%</b>');
						  $('#modal-ahorcado').modal("show");
						  /$('#calificar').addClass("d-none");/
						  $('canvas').removeClass("cursor");
